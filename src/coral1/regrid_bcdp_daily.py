...
outdir = basedir+'data/%s/%s/raw/xinru/' % (cmip_var, runname) # note: "data" is a symlink to /raid8/pkalmus/data/coral/data/
        ...
        for mymember in members:
            ...
            if not dryRun:
                ...
                for scenario in scenarios:
                    outfilename = outdir+mysource+'_'+mymember+'_'+grid+'_'+scenario+'_'+interpolate_method+'_DAY1984.nc'
                ...
                # get historical
                query = dict(source_id=mysource, member_id=mymember, experiment_id='historical', variable_id=cmip_var, table_id=table, grid_label=grid)
                try:
                ...
                if grid=='gn':
                    da_hist = ens.regrid(output_grid=grid_ds, backend='esmf', method=interpolate_method, ignore_degenerate=True).first
                else:
                    da_hist = ens.first

                # only need from 1984
                da_hist = da_hist.sel(time=slice(cftime.DatetimeNoLeap(1984, 1, 1), None ))
                ...
                for scenario in scenarios:
                        ...
                        try:
                            ...
                            if grid=='gn':
                                ...
                                da = ens.regrid(output_grid=grid_ds, backend='esmf', method=interpolate_method, ignore_degenerate=True).first
                            else:
                                da = ens.first
...
print('NOW DELETE GR,GR1 DUPLICATES FROM LIST! %s' % daylistfilename)
print('done.')
