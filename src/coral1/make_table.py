...
for (ind, scenario) in enumerate(scenarios): # reload each file 4 times, once per scenario
    ...
    for return_year in return_years:
        if do_weights:
            experiment = '%s_%iyrs_DHW%1.1f' % ('ssp585', return_year, DHW_threshold) # this will be replaced by individual model if we're in single_mod
        else:
            experiment = 'FLAT_%s_%iyrs_DHW%1.1f' % ('ssp585', return_year, DHW_threshold) # this will be replaced by individual model if we're in single_mod
...
# do it for temps
outstr = outstr+'	\multicolumn{10}{|c|}{Global mean surface temperature (\\degreec) }\\\\ \n'
outstr = outstr+'	\hline\n'
for (ind, scenario) in enumerate(scenarios): # reload each file 4 times, once per scenario
...
outstr = outstr+'	& \multicolumn{3}{|c||}{TD1Y} & \multicolumn{3}{|c||}{TD3Y} & \multicolumn{3}{|c|}{TD5Y} \\\\ \n'
outstr = outstr+'	\hline\n'
outstr = outstr+'	SSP & 1.5\\degreec & 1.7\\degreec & 2.0\\degreec & 1.5\\degreec & 1.7\\degreec & 2.0\\degreec  & 1.5\\degreec & 1.7\\degreec & 2.0\\degreec  \\\\ \n'
outstr = outstr+'	\hline\hline\n'    
...
outstr = outstr+'	\multicolumn{10}{|c|}{Number of 1 km$^2$ reef locations remaining below threshold }\\\\ \n'
outstr = outstr+'	\hline\n'
for (ind, scenario) in enumerate(scenarios): # reload each file 4 times, once per scenario
    if ind==0: # no GMST for ssp126
        continue
