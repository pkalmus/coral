...
# filetag = 'dhwbma_%i-%i_%i-%i' % (climstartyear_weight, climendyear_weight, meanstart, meanend)

# reefdir = basedir+'data/tos/%s/reef/' % (runname) 
# outdir = weightdir
# outdir = projectdir+'coral2/output/weighting/'


indir = basedir+'data/tos/coral2/%s/reef/' % (runname) # note: "data" is a symlink to /raid8/pkalmus/data/coral/data/
weightdir = indir
valdir = basedir+'data/tos/coral2/%s/val/' % (runname) 
...
obs_ds = xr.open_dataset(obs_filename)
# calculate obs mmm climatology from HadISST
clim_ds = obs_ds.sel(time=slice(climstartyear_weight, climendyear_weight))
# take annual mean
...
for (modelind, model) in enumerate(models):
    ...
    filename = indir+modsspstr+'_reef.nc'
...
aa = weights_ds.pval.copy()
aa.values = np.random.uniform(size=weights_ds.pval.shape)
#aa.values = np.random.rand(weights_ds.pval.shape[0], weights_ds.pval.shape[1])
#aa.values = np.ones(weights_ds.pval.shape)

#aa = aa.where(~np.isnan(weights_ds.pval.values))
rand_tos_da = average_da(all_ds.tos, dim='model_index', weights=aa)
rand_tos_da = rand_tos_da.sel(time=slice(valstartyear, valendyear))
...
for loopind in obs_ds.index.values: # validation loop
    ...
    if ~np.isnan(mmm):
        ...
        rand_dhw_ts = get_dhw(mmm, rand_tos_da.values[loopind,:])
        ...
        randdiffs.append(randdiff)

data = np.array(flatdiffs)
plotter.hist(data, 'degree heating weeks', valdir+'hist_flatdiffs_dhwmax_%s_%i.png' % (filetag, min_models), title='flat', xlim=[-15,40], logy=True)
data = np.array(weightdiffs)
plotter.hist(data, 'degree heating weeks', valdir+'hist_weightdiffs_dhwmax_%s_%i.png' % (filetag, min_models), title='arithmetic weights', xlim=[-15,40], logy=True)
data = np.array(randdiffs)
plotter.hist(data, 'degree heating weeks', valdir+'hist_randdiffs_dhwmax_%s_%i.png' % (filetag, min_models), title='random', xlim=[-15,40], logy=True)
